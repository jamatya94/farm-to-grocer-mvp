...
export default function DemoPage() {
  ...
  const choose = (role: "vendor_owner" | "grocer_owner" | "platform_admin", profileId: string) => {
    setSession({ role, profileId })
    if (role === "vendor_owner") window.location.assign("/vendor/dashboard")
    if (role === "grocer_owner") window.location.assign("/buyer/dashboard")
    if (role === "platform_admin") window.location.assign("/admin/dashboard")
  }
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(247,251,245,0.96),rgba(251,247,240,0.98)_46%,rgba(246,240,229,1)_100%)]">
      <div className="mx-auto max-w-7xl px-4 pb-20 pt-5 sm:px-6 lg:px-8">
        <header className="rounded-[32px] border border-white/60 bg-white/72 px-5 py-4 shadow-[0_36px_90px_-60px_rgba(20,35,28,0.45)] backdrop-blur-md">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="soft" size="sm" onClick={reset}>Reset data</Button>
            </div>
          </div>
        </header>
        ...
        <section className="mt-6 rounded-[34px] border border-white/70 bg-[linear-gradient(145deg,rgba(255,255,255,0.8),rgba(255,255,255,0.56))] px-6 py-8 shadow-[0_42px_100px_-68px_rgba(20,35,28,0.55)] backdrop-blur-md md:px-8 md:py-10">
          <p className="section-kicker">Choose a role</p>
          <h1 className="mt-4 text-4xl font-semibold tracking-[-0.07em] text-[hsl(var(--foreground))] md:text-5xl">Step directly into the seeded Baltimore + DMV demo.</h1>
          <p className="mt-4 max-w-3xl text-base leading-8 text-[hsl(var(--foreground-soft))]">This environment skips public signup so you can experience the workflow immediately. Select a vendor, grocer, or admin profile and move through the exact role-based paths we agreed to demo.</p>
        </section>
        ...
        <div className="mt-8 grid gap-6 xl:grid-cols-3">
          ...
          >
            {vendorOptions.map((vendor) => (
              <button key={vendor.id} onClick={() => choose("vendor_owner", vendor.id)} className="flex w-full items-center justify-between rounded-3xl border border-[hsl(var(--border))] bg-white px-5 py-4 text-left transition hover:bg-[hsl(var(--accent))]">
                <div>
                  <p className="font-medium text-[hsl(var(--foreground))]">{vendor.name}</p>
                  <p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">
  {getCountyName(vendor.primaryCountyId)}
    ...
    : ""}
          ...
          >
            {grocerOptions.map((grocer) => (
              <button key={grocer.id} onClick={() => choose("grocer_owner", grocer.id)} className="flex w-full items-center justify-between rounded-3xl border border-[hsl(var(--border))] bg-white px-5 py-4 text-left transition hover:bg-[hsl(var(--accent))]">
                <div>
                  <p className="font-medium text-[hsl(var(--foreground))]">{grocer.name}</p>
                  <p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">
  {getCountyName(grocer.primaryCountyId)}
  {Array.isArray(grocer.categoriesOfInterest) && grocer.categoriesOfInterest.length > 0
    ...
    : ""}
          ...
          >
            <button onClick={() => choose("platform_admin", "admin-platform")} className="flex w-full items-center justify-between rounded-3xl border border-[hsl(var(--border))] bg-white px-5 py-4 text-left transition hover:bg-[hsl(var(--accent))]">
              <div>
                <p className="font-medium text-[hsl(var(--foreground))]">Platform Admin</p>
                <p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">Cross-role visibility into vendors, grocers, connections, and orders.</p>
              </div>
              <ArrowRight className="h-4 w-4 text-[hsl(var(--muted-foreground))]" />
            </button>
          </RolePanel>
    ...
    <section className="rounded-[30px] border border-white/70 bg-white/78 p-5 shadow-[0_35px_90px_-65px_rgba(20,35,28,0.45)] backdrop-blur-sm md:p-6">
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm leading-7 text-[hsl(var(--foreground-soft))]">{description}</p>
        </div>
